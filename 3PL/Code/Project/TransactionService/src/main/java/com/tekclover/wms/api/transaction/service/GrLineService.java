...
//							createdGRLine.getItemCode(), 1L, 0L);
     ...
     * @return
     */
    public List<GrLineV2> getGrLineV2forDelete(String companyCode, String languageId, String plantId,
                                               String warehouseId, String refDocNumber) {
        List<GrLineV2> grLine =
                grLineV2Repository.findByLanguageIdAndCompanyCodeAndPlantIdAndWarehouseIdAndRefDocNumberAndDeletionIndicator(
                        languageId,
                        companyCode,
                        plantId,
                        warehouseId,
    ...
    public List<GrLineV2> createGrLineV2(@Valid List<AddGrLineV2> newGrLines, String loginUserID)
        ...
        try {
//            List<AddGrLineV2> dupGrLines = getDuplicatesV2(newGrLines);
//            log.info("-------dupGrLines--------> " + dupGrLines);
//            if (dupGrLines != null && !dupGrLines.isEmpty()) {
//                newGrLines.removeAll(dupGrLines);
//                newGrLines.add(dupGrLines.get(0));
...
//            }
            ...
            for (AddGrLineV2 newGrLine : newGrLines) {
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    if (packBarcode.getCbm() == null) {
                        packBarcode.setCbm(0D);
                    }
                    if (newGrLine.getGoodReceiptQty() == null) {
                        newGrLine.setGoodReceiptQty(0D);
                    }
                    dbGrLine.setCbmQuantity(packBarcode.getCbm() / newGrLine.getGoodReceiptQty());
                    dbGrLine.setBillStatus("GR");

                    dbGrLine.setCbm(packBarcode.getCbm());
                    dbGrLine.setCbmQuantity(packBarcode.getCbmQuantity());

                    if (newGrLine.getHeight() != null) {
                        dbGrLine.setHeight(Math.cbrt(newGrLine.getHeight()));
                    }
                    if (newGrLine.getLength() != null) {
                        dbGrLine.setLength(Math.cbrt(newGrLine.getLength()));
                    }
                    if (newGrLine.getWidth() != null) {
                        dbGrLine.setWidth(Math.cbrt(newGrLine.getWidth()));
                    }
...
//                Long binClassId = 1L;                   //currently(21-8-2023) hard code to get the output
    ...
    private void createInventoryMovement(GrLineV2 createdGRLine, InventoryV2 createdinventory) {
        InventoryMovementV2 inventoryMovement = new InventoryMovementV2();
        BeanUtils.copyProperties(createdGRLine, inventoryMovement, CommonUtils.getNullPropertyNames(createdGRLine));
        inventoryMovement.setCompanyCodeId(createdGRLine.getCompanyCode());
        ...
        inventoryMovement.setDocumentDate(new Date());

        Double dbInventory = inventoryV2Repository.getOpeningStock(createdGRLine.getItemCode(),createdGRLine.getBusinessPartnerCode());
        if(dbInventory != null) {
            inventoryMovement.setOpeningStock(dbInventory);
        } else {
            inventoryMovement.setOpeningStock(0D);
        }
        if(createdGRLine.getCbm() != null) {
            inventoryMovement.setInboundQuantity(createdGRLine.getCbm());
        } else {
            inventoryMovement.setInboundQuantity(0D);
        }

        if (inventoryMovement.getOutboundQuantity() == null) {
            inventoryMovement.setOutboundQuantity(0D);
        }

        Double closingStock = (inventoryMovement.getOpeningStock() + inventoryMovement.getInboundQuantity()) - inventoryMovement.getOutboundQuantity();

        inventoryMovement.setClosingStock(closingStock);
        inventoryMovement.setInventoryUom(createdGRLine.getGrUom());
        inventoryMovement.setDescription(createdGRLine.getItemDescription());
        inventoryMovement.setStatusId(17L);
        ...
        statusDescription = stagingLineV2Repository.getStatusDescription(17L, createdGRLine.getLanguageId());
        inventoryMovement.setStatusDescription(statusDescription);
    ...
    private InventoryV2 createInventory(GrLineV2 createdGRLine) {
        InventoryV2 dbInventory = stagingLineV2Repository.getInventory(createdGRLine.getItemCode(),
                createdGRLine.getCompanyCode(),
                createdGRLine.getPlantId(),
                createdGRLine.getWarehouseId(),
        ...
        if (dbInventory == null) {
            ...
            inventory.setPartnerId(createdGRLine.getBusinessPartnerCode());
    ...
    public void deleteGrLine(String companyCodeId, String plantId, String languageId,
        ...
        InventoryV2 dbInventory = inventoryService.getInventory(companyCodeId, plantId, languageId, warehouseId,
                packBarcodes, itemCode, grLine.getInterimStorageBin(), grLine.getManufacturerCode());
        InventoryMovementV2 dbInventoryMovement = inventoryMovementService.getInventoryMovement(companyCodeId, plantId, languageId,
                warehouseId, 1L, 1L, packBarcodes, itemCode, "1", grLine.getGoodsReceiptNo());
        List<GrLineV2> dbGrLine = getGrLineV2forDelete(companyCodeId, languageId, plantId, warehouseId, refDocNumber);
        if (grLine != null) {
            grLine.setDeletionIndicator(1L);
            grLine.setUpdatedBy(loginUserID);
            ...
            if (dbGrLine == null || dbGrLine.isEmpty()) {
                List<GrHeaderV2> dbGrHeaderList = grHeaderService.getGrHeaderV2(warehouseId, goodsReceiptNo, caseCode, companyCodeId, languageId, plantId, refDocNumber);
                if (dbGrHeaderList != null) {
                    for(GrHeaderV2 dbGrHeader : dbGrHeaderList) {
                        dbGrHeader.setDeletionIndicator(1L);
