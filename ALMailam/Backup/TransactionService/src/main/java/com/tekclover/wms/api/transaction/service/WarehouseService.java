...
//				apiLine.setOrigin(b2bTransferInLine.getOrigin());
	...
	private CycleCountHeader savePeriodic (Periodic periodic) {
		try {
			...
			apiHeader.setIsCancelled(perpetualHeaderV1.getIsCancelled());
			apiHeader.setIsCompleted(perpetualHeaderV1.getIsCompleted());
			apiHeader.setCycleCountCreationDate(perpetualHeaderV1.getCycleCountCreationDate());
...
//====================================================OutBound V2===========================================================================
	 ...
	 * @return
	 */
	public ShipmentOrderV2 postSOV2(ShipmentOrderV2 shipmenOrder, boolean isRerun) {
		log.info("ShipmenOrder received from External: " + shipmenOrder);
	 ...
	 * @return
	 */
	public ReturnPOV2 postReturnPOV2(ReturnPOV2 returnPO) {
		log.info("ReturnPOHeader received from External: " + returnPO);
	 ...
	 * @return
	 */
	public InterWarehouseTransferOutV2 postInterWarehouseTransferOutboundV2(InterWarehouseTransferOutV2 interWarehouseTransfer) {
		log.info("InterWarehouseTransferHeader received from External: " + interWarehouseTransfer);
	 ...
	 * @return
	 */
	public OutboundOrderV2 postSalesInvoice(SalesInvoice salesInvoice) {
	...
	private OutboundOrderV2 saveSalesOrderV2(@Valid SalesOrderV2 salesOrder) {
		try {
			...
			try {
				Date reqDate = DateUtils.convertStringToDate(salesOrderHeader.getRequiredDeliveryDate());
	 ...
	 * @return
	 */
	private OutboundOrderV2 saveReturnPOV2(ReturnPOV2 returnPO) {
		try {
	 ...
	 * @return
	 */
	private OutboundOrderV2 saveIWHTransferV2(InterWarehouseTransferOutV2 iWhTransferOutV2) {
		try {
			...
			apiHeader.setBranchCode(interWhTransferOutHeader.getToBranchCode());
			apiHeader.setCompanyCode(interWhTransferOutHeader.getToCompanyCode());
...
//			apiHeader.setWarehouseID(interWhTransferOutHeader.getFromWhsID());
	...
	//Save SalesInvoice
	private OutboundOrderV2 saveSalesInvoice(SalesInvoice salesInvoice) {
		try {
...
//			}
	...
	private StockAdjustment saveStockAdjustment(StockAdjustment stockAdjustment) {
		try {
			...
			StockAdjustment dbSA = new StockAdjustment();
			...
			stockAdjustment.setMiddlewareTable(dbSA.getMiddlewareTable());

			// Get Warehouse
			Optional<Warehouse> dbWarehouse =
					warehouseRepository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndDeletionIndicator(
							dbSA.getCompanyCode(), dbSA.getBranchCode(),
							"EN", 0L);
			log.info("dbWarehouse: " + dbWarehouse);
			validateWarehouseId(dbWarehouse.get().getWarehouseId());
			stockAdjustment.setWarehouseId(dbWarehouse.get().getWarehouseId());

			stockAdjustment.setRefDocType("Stock Adjustment");
			stockAdjustment.setOrderReceivedOn(new Date());
			stockAdjustment.setOrderProcessedOn(new Date());

			IKeyValuePair iKeyValuePair = outboundOrderV2Repository.getV2Description(
					stockAdjustment.getCompanyCode(), stockAdjustment.getBranchCode(), dbWarehouse.get().getWarehouseId());
			if (iKeyValuePair != null) {
				stockAdjustment.setCompanyDescription(iKeyValuePair.getCompanyDesc());
				stockAdjustment.setPlantDescription(iKeyValuePair.getPlantDesc());
				stockAdjustment.setWarehouseDescription(iKeyValuePair.getWarehouseDesc());
			}

			if (stockAdjustment != null) {
				try {
					stockAdjustment.setProcessedStatusId(0L);
					log.info("stockAdjustment: " + stockAdjustment);
					StockAdjustment createdOrder = stockAdjustmentService.createStockAdjustment(stockAdjustment);
					log.info("StockAdjustment Order Success: " + createdOrder);
					return createdOrder;
				} catch (Exception e) {
					stockAdjustment.setProcessedStatusId(100L);
					log.info("StockAdjustment: " + stockAdjustment);
					StockAdjustment createdOrder = stockAdjustmentService.createStockAdjustment(stockAdjustment);
