            ...
            for (AddGrLineV2 newGrLine : newGrLines) {
                ...
                for (PackBarcode packBarcode : newGrLine.getPackBarcodes()) {
                    ...
                    if (packBarcode.getCbm() != null) {
                        cbm = packBarcode.getCbm();
                    }
                    if (newGrLine.getGoodReceiptQty() != null) {
                        grQty = newGrLine.getGoodReceiptQty();
                    }

                    if (packBarcode.getCbmQuantity() != null) {
                        cbmPerQty = packBarcode.getCbmQuantity();
                    } else {
                        cbmPerQty = cbm / grQty;
                    }
...
//                ImBasicData1 itemCodeCapacityCheck = mastersService.getImBasicData1ByItemCodeV2(
//                        itemCode, languageId, companyCode, plantId, warehouseId, createdGRLine.getManufacturerName(),
//                        authTokenForMastersService.getAccess_token());
...
//                    StorageBinV2 storageBin = storageBinRepository.getStorageBin(companyCode, plantId, languageId, warehouseId, proposedStorageBin);
                    ...
                    if (createdGRLine.getCbm() != null && capacityCheck) {
...
//                            if (!capacityCheck) {
//                                stBinList = storageBinRepository.getStorageBinListByCompanyBranchNonCBM(itemCode, warehouseId, companyCode, plantId, languageId,
//                                        binClassId, createdGRLine.getManufacturerName());
//                            }
                        }
...
//                                updateEmptyStorageBinStatus(proposedStBin,
//                                        companyCode,
//                                        plantId,
//                                        languageId,
//                                        warehouseId,
...
//                                        loginUserID, authTokenForMastersService.getAccess_token());
                        ...
                        if (!capacityCheck) {
                            ...
                            if (existingBinItemCheck != null && !existingBinItemCheck.isEmpty()) {
                                proposedStorageBin = existingBinItemCheck.get(0).getProposedStorageBin();
                                putAwayHeader.setProposedStorageBin(proposedStorageBin);
                                log.info("Existing PutawayCreate ProposedStorageBin -->D : " + proposedStorageBin);
                                cbm = 0D;   //break the loop
                            }
                            if (existingBinItemCheck == null || existingBinItemCheck.isEmpty() || existingBinItemCheck.size()==0) {
                                List<String> existingProposedPutawayStorageBin = putAwayHeaderService.getPutawayHeaderExistingBinCheckV2(companyCode, plantId, languageId, warehouseId);
                                storageBinPutAway.setStorageBin(existingProposedPutawayStorageBin);
                                proposedNonCbmStorageBin = mastersService.getStorageBinNonCbm(storageBinPutAway, authTokenForMastersService.getAccess_token());
                                if (proposedNonCbmStorageBin != null) {
...
//                                        loginUserID, authTokenForMastersService.getAccess_token());
    ...
    private InventoryV2 createInventoryV2(GrLineV2 createdGRLine) {
        InventoryV2 dbInventory = inventoryV2Repository.findByCompanyCodeIdAndPlantIdAndLanguageIdAndWarehouseIdAndItemCodeAndManufacturerCodeAndPackBarcodesAndDeletionIndicator(
                createdGRLine.getCompanyCode(),
                createdGRLine.getPlantId(),
                createdGRLine.getLanguageId(),
                createdGRLine.getWarehouseId(),
                ...
                createdGRLine.getPackBarcodes(), 0L);
        if (dbInventory == null) {
...
//            ImBasicData1 itemCodeCapacityCheck = mastersService.getImBasicData1ByItemCodeV2(
//                    createdGRLine.getItemCode(), createdGRLine.getLanguageId(),
//                    createdGRLine.getCompanyCode(), createdGRLine.getPlantId(),
//                    createdGRLine.getWarehouseId(), createdGRLine.getManufacturerName(),
//                    authTokenForMastersService.getAccess_token());
