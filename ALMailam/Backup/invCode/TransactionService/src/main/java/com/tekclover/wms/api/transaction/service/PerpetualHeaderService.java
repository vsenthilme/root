...
//	}
     ...
     * @throws java.text.ParseException
     */
    public List<PerpetualHeaderV2> findPerpetualHeaderV2(SearchPerpetualHeaderV2 searchPerpetualHeader)
            throws ParseException, java.text.ParseException {
        if (searchPerpetualHeader.getStartCreatedOn() != null && searchPerpetualHeader.getStartCreatedOn() != null) {
            Date[] dates = DateUtils.addTimeToDatesForSearch(searchPerpetualHeader.getStartCreatedOn(),
                    searchPerpetualHeader.getEndCreatedOn());
        ...
        for(CycleCountLine cycleCountLine : cycleCountHeader.getLines()) {
            ...
            if(dbInventoryList != null) {
                for(IInventoryImpl dbInventory : dbInventoryList) {
                    ...
                    dbPerpetualLine.setSpecialStockIndicator(dbPerpetualLine.getSpecialStockIndicator());
                    dbPerpetualLine.setInventoryQuantity(dbInventory.getInventoryQuantity());
                    dbPerpetualLine.setInventoryUom(dbInventory.getInventoryUom());
