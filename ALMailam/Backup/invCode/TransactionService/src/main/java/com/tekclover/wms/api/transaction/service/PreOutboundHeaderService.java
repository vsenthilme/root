...
//        OutboundHeaderV2 outboundHeader = createOutboundHeaderV2(createdPreOutboundHeader, createdOrderManagementHeader.getStatusId());
    ...
    private void updateStatusAs48ForPickupHeaderCreateSuccess(String companyCodeId, String plantId, String languageId, String warehouseId,
                                                              ...
                                                              String preOutboundNo, String refDocNumber, String partnerCode) throws InvocationTargetException, IllegalAccessException {
        ...
        for(OrderManagementLineV2 orderManagementLine : orderManagementLineV2List){
            ...
            if (currentTime < 15) {
                ...
                if (warehouseId.equalsIgnoreCase("100")) {
                    ...
                    List<String> hhtUserList = preOutboundHeaderV2Repository.getHHTUserList(OB_ORD_TYP_ID);
                    ...
                    if (hhtUserList != null) {
                        List<PickupHeaderV2> pickupHeaderAssignedPickerIdList = pickupHeaderService.getPickupHeaderAutomation(companyCodeId, plantId, languageId, warehouseId, hhtUserList);
...
//                            if (pickupHeaderAssignedPickerIdList == null || pickupHeaderAssignedPickerIdList.isEmpty()) {
                        ...
                        for (String hhtUser : hhtUserList) {
                            List<PickupHeaderV2> pickupHeaderList = pickupHeaderService.getPickupHeaderAutomationV2(companyCodeId, plantId, languageId, warehouseId, hhtUser);
                            log.info("pickupHeaderList: " + pickupHeaderList);

                            if (pickupHeaderList == null || pickupHeaderList.isEmpty()) {
                                assignPickerIdCount = assignPickerIdCount + 1;

                                if (assignPickerIdCount == 1) {
                                    assignPickerId = hhtUser;
                                    log.info("assignedPickerId: " + assignPickerId);
                                }
                                if (assignPickerIdCount > 1) {
                                    if (hhtUserCountInPickupHeader == 0 || hhtUserCountInPickupHeader == 1) {
                                        hhtUserCountInPickupHeader = 2L;
                                    }
                                    if (hhtUserCountInPickupHeader == assignPickerIdCount) {
                                        break outerLoop;
                                    }
                                }
                            }
                            if (pickupHeaderList != null && !pickupHeaderList.isEmpty()) {
                                assignPickerIdNotNullCount = assignPickerIdNotNullCount + 1;
                                log.info("assignPickerIdNotNullCount: " + assignPickerIdNotNullCount);
                            }
                        }
                        ...
                        if (pickupLineList != null) {
                            assignPickerId = pickupLineList.get(0).getAssignedPickerId();
                            log.info("assignedPickerId: " + assignPickerId);
                        }
//                            }
                        if (hhtUserCount == assignPickerIdNotNullCount) {
                            if (pickupHeaderAssignedPickerIdList != null && !pickupHeaderAssignedPickerIdList.isEmpty()) {
                                log.info("hhtUsrCount == assignPickerIdNotNullCount: " + hhtUserCount + ", " + assignPickerIdNotNullCount);

                                pickupHeaderAssignedPickerIdList.stream().sorted(Comparator.comparing(PickupHeaderV2::getPickupCreatedOn)).collect(Collectors.toList());
                                assignPickerId = pickupHeaderAssignedPickerIdList.get(0).getAssignedPickerId();
                                log.info("assignedPickerId: " + assignPickerId);
                            }
                        }
                    }
                ...
                if (warehouseId.equalsIgnoreCase("200")) {
                    ...
                    List<String> hhtUserList = preOutboundHeaderV2Repository.getHHTUserListByLevelId(LEVEL_ID);
                    log.info("hhtUserList: " + hhtUserList);
                    ...
                    if (hhtUserList != null) {
                        ...
                        List<PickupHeaderV2> pickupHeaderAssignedPickerIdList = pickupHeaderService.getPickupHeaderAutomation(companyCodeId, plantId, languageId, warehouseId, hhtUserList);
                        log.info("PickUpHeader: " + pickupHeaderAssignedPickerIdList);
...
//                            if (pickupHeaderAssignedPickerIdList == null || pickupHeaderAssignedPickerIdList.isEmpty()) {
                        ...
                        for (String hhtUser : hhtUserList) {
                            ...
                            if (pickupHeaderList == null || pickupHeaderList.isEmpty()) {
                                ...
                                if (assignPickerIdCount > 1) {
                                    if (hhtUserCountInPickupHeader == 0L || hhtUserCountInPickupHeader == 1L) {
                                        hhtUserCountInPickupHeader = 2L;
                                    }
                                    if (hhtUserCountInPickupHeader == assignPickerIdCount) {
                                        break outerLoop;
...
//        preOutboundLine.setCreatedOn(new Date());
    ...
    private OrderManagementLineV2 createOrderManagementLineV2(String companyCodeId, String plantId, String languageId, String preOutboundNo,
        ...
        if(inventoryV2 != null && !inventoryV2.isEmpty()) {
            orderManagementLine.setPartnerItemBarcode(inventoryV2.get(0).getBarcodeId());
...
//            List<String> storageSectionIds = Arrays.asList("ZD"); //ZD
    ...
    private OrderManagementLineV2 createOrderManagementV2(String companyCodeId, String plantId, String languageId,
                                                          ...
                                                          String warehouseId, String itemCode, Double ORD_QTY) {
        List<InventoryV2> stockType1InventoryList = inventoryService.
