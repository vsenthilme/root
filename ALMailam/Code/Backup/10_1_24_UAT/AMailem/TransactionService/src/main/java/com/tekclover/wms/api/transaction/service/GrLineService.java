...
//            grLine.setUpdatedOn(DateUtils.getCurrentKWTDateTime());
    ...
    public List<GrLineV2> deleteGrLineV2(String companyCode, String plantId, String languageId,
        ...
        List<GrLineV2> dbGrLine = grLineV2Repository.findByCompanyCodeAndPlantIdAndLanguageIdAndWarehouseIdAndRefDocNumberAndDeletionIndicator(
                companyCode, plantId, languageId, warehouseId, refDocNumber, 0L);

        if (dbGrLine != null) {
            for (GrLineV2 grLine : dbGrLine) {
                grLine.setDeletionIndicator(1L);
                grLine.setUpdatedBy(loginUserID);
                grLine.setUpdatedOn(DateUtils.getCurrentKWTDateTime());
...
//        else {
//            throw new EntityNotFoundException("Error in deleting Id: warehouseId:" + warehouseId +
//                    ",refDocNumber: " + refDocNumber + ", companyCode " + companyCode + " doesn't exist ");
//        }
        return grLineV2s;
    }
