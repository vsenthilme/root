...
public class HandlingEquipmentIdSpecification implements Specification<HandlingEquipmentId> {
    ...
    public Predicate toPredicate(Root<HandlingEquipmentId> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (findHandlingEquipmentId.getHandlingEquipmentId() != null && !findHandlingEquipmentId.getHandlingEquipmentId().isEmpty()) {
            final Path<DeferredImportSelector.Group> group = root.<DeferredImportSelector.Group>get("handlingEquipmentId");
            predicates.add(group.in(findHandlingEquipmentId.getHandlingEquipmentId()));
        }
