...
public class HandlingUnitIdSpecification implements Specification<HandlingUnitId> {
    ...
    public Predicate toPredicate(Root<HandlingUnitId> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (findHandlingUnitId.getHandlingUnitId() != null && !findHandlingUnitId.getHandlingUnitId().isEmpty()) {
            final Path<DeferredImportSelector.Group> group = root.<DeferredImportSelector.Group>get("handlingUnitId");
            predicates.add(group.in(findHandlingUnitId.getHandlingUnitId()));
        }
