...
public class ImStrategiesSpecification implements Specification<ImStrategies> {
    ...
    public Predicate toPredicate(Root<ImStrategies> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (searchImStrategies.getStrategeyTypeId() != null && !searchImStrategies.getStrategeyTypeId().isEmpty()) {
            final Path<DeferredImportSelector.Group> group = root.<DeferredImportSelector.Group> get("strategeyTypeId");
            predicates.add(group.in(searchImStrategies.getStrategeyTypeId()));
        }
