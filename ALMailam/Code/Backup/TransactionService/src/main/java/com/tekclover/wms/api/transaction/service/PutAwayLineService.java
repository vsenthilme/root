...
//    }
    ...
    public List<PutAwayLineV2> putAwayLineConfirmV2(@Valid List<PutAwayLineV2> newPutAwayLines, String loginUserID)
        ...
        List<PutAwayLineImpl> putAwayLineImplList = new ArrayList<>();
        log.info("newPutAwayLines to confirm : " + newPutAwayLines);

        if(newPutAwayLines != null && !newPutAwayLines.isEmpty()){
            for(PutAwayLineV2 putAwayLineV2 : newPutAwayLines){
                PutAwayLineImpl putAwayLineimpl = new PutAwayLineImpl();
                putAwayLineimpl.setCompanyCode(putAwayLineV2.getCompanyCode());
                putAwayLineimpl.setPlantId(putAwayLineV2.getPlantId());
                putAwayLineimpl.setLanguageId(putAwayLineV2.getLanguageId());
                putAwayLineimpl.setWarehouseId(putAwayLineV2.getWarehouseId());
                putAwayLineimpl.setItemCode(putAwayLineV2.getItemCode());
                putAwayLineimpl.setManufacturerName(putAwayLineV2.getManufacturerName());
                putAwayLineimpl.setPutawayNumber(putAwayLineV2.getPutAwayNumber());
                putAwayLineimpl.setLineNo(putAwayLineV2.getLineNo());
                putAwayLineimpl.setPutawayQty(putAwayLineV2.getPutawayConfirmedQty());
                putAwayLineimpl.setRefDocNumber(putAwayLineV2.getRefDocNumber());
                putAwayLineimpl.setPreInboundNo(putAwayLineV2.getPreInboundNo());
                putAwayLineImplList.add(putAwayLineimpl);
            }
            log.info("PutAwayImpl : " + putAwayLineImplList);
        }
        AuthToken authTokenForMastersService = authTokenService.getMastersServiceAuthToken();
        try {
...
//                            PutAwayHeaderV2 dbputAwayHeader = putAwayHeaderService.getPutAwayHeaderV2ForPutAwayLine(
//                                    createdPutAwayLine.getCompanyCode(),
//                                    createdPutAwayLine.getPlantId(),
//                                    createdPutAwayLine.getLanguageId(),
//                                    createdPutAwayLine.getWarehouseId(),
//                                    createdPutAwayLine.getPreInboundNo(),
//                                    createdPutAwayLine.getRefDocNumber(),
//                                    String.valueOf(createdPutAwayLine.getLineNo()),
//                                    createdPutAwayLine.getItemCode(),
...
//                                    );
                        if (putAwayHeader != null) {
                            ...
                            if (createdPutAwayLine.getPutawayConfirmedQty() < putAwayHeader.getPutAwayQuantity()) {
                                List<PutAwayLineImpl> filteredlist = putAwayLineImplList
                                        .stream()
                                        .filter(a ->
                                ...
                                log.info("PutawayLine Impl filtered List: " + filteredlist);
                                if (filteredlist != null && !filteredlist.isEmpty()) {
                                    Double putawayQty = filteredlist.stream().mapToDouble(a -> a.getPutawayQty()).sum();
                                    Double dbPutawayQty = Double.valueOf(putAwayHeader.getReferenceField3());
                                    Double dbAssignedPutawayQty = Double.valueOf(putAwayHeader.getReferenceField2());
...
//                                                                                    createdPutAwayLine.getLineNo());
                                    Double totalPutawayCnfQty = putawayQty;
                                    if (dbPutawayQty != null) {
                                        totalPutawayCnfQty = totalPutawayCnfQty + dbPutawayQty;
                                    }
                                    log.info("pa_cnf_qty,tot_pa_cnf_qty,created_pa_line_cnf_qty,partial_pa_header_pa_qty,pa_header_pa_qty : "
                                            + putawayQty + ", " + totalPutawayCnfQty + ", " + createdPutAwayLine.getPutawayConfirmedQty()
                                            + ", " + putAwayHeader.getPutAwayQuantity() + ", " + putAwayHeader.getReferenceField2());
                                    if (totalPutawayCnfQty > dbAssignedPutawayQty) {
                                    ...
                                    if (totalPutawayCnfQty <= dbAssignedPutawayQty) {
                                        ...
                                        if(!putAwayHeader.getReferenceField4().equalsIgnoreCase("1")){
