...
public class DeliveryHeaderSpecification implements Specification<DeliveryHeader> {
    ...
    public Predicate toPredicate(Root<DeliveryHeader> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
        ...
        if (searchDeliveryHeader.getRefDocNo() != null && !searchDeliveryHeader.getRefDocNo().isEmpty()) {
            final Path<DeferredImportSelector.Group> group = root.<DeferredImportSelector.Group> get("refDocNo");
            predicates.add(group.in(searchDeliveryHeader.getRefDocNo()));
        }
