...
screenid=3056;
  ...
  getpalletid() {
    ...
    this.spin.show();
    this.sub.add(this.service.searchv2({ refDocNumber: [this.refDocNumber],companyCodeId:[this.auth.companyId],plantId:[this.auth.plantId],warehouseId:[this.auth.warehouseId],statusId:[20] }).subscribe(res => {
      this.spin.hide();
  ...
  reverse() {
   // this.packBarcodes=this.referenceField5;
    ...
    this.selectedputaway.forEach(x => {
      this.sub.add(this.service.reversev2(this.refDocNumber, x.packBarcodes,this.auth.companyId,this.auth.plantId,this.auth.warehouseId,this.auth.languageId).subscribe(res => {
        this.spin.hide();
...
onItemchnage(){
 ...
 obj.itemCode=[this.referenceField5.id];
  this.sub.add(this.service.searchLinev2(obj).subscribe(res => {
    this.putaway=res;
