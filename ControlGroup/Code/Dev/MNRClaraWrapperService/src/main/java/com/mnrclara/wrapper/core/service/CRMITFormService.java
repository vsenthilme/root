...
import com.mnrclara.wrapper.core.model.crm.itform.MatterITForm000;
	...
	public ITForm003Att getITForm003Att(String inquiryNo, Long classID, String language, String itFormNo, Long itFormID,
		...
		try {
			...
			UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getCRMServiceUrl() + "/itform003/attorney")
					...
					.queryParam("itFormNo", itFormNo).queryParam("itFormID", itFormID);
			HttpEntity<?> entity = new HttpEntity<>(headers);
			ResponseEntity<ITForm003Att> result = getRestTemplate().exchange(builder.toUriString(), HttpMethod.GET, entity,
					ITForm003Att.class);
			log.info("result : " + result.getBody());
			return result.getBody();
		} catch (Exception e) {
	...
	public ITForm007 getITForm007(String language, Long classID, String matterNumber, String clientId, String itFormNo, Long itFormID,
		...
		try {
			...
			UriComponentsBuilder builder = UriComponentsBuilder.fromHttpUrl(getCRMServiceUrl() + "/itform007/id")
					.queryParam("matterNumber", matterNumber).queryParam("classID", classID).queryParam("language", language)
					.queryParam("clientId", clientId).queryParam("itFormNo", itFormNo).queryParam("itFormID", itFormID);
			HttpEntity<?> entity = new HttpEntity<>(headers);
