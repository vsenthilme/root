	...
	public void scheduleARAgingReport() throws Exception {
		try {
			...
			List<com.mnrclara.api.accounting.model.prebill.MatterGenAcc> matterGenAccList = 
					matterGenAccRepository.findByClassId(Arrays.asList(1L, 2L));
			Set<String> setMatterNumbers = new HashSet<String>();
...
//	invoiceHeaderRepository.getARInvoiceList(billingReportInput.getClassId(),billingReportInput.getFromPostingDate(), billingReportInput.getToPostingDate());
	...
	invoiceHeaderRepository.updateARInvoiceFeeBilled(billingReportInput.getClassId(),billingReportInput.getFromPostingDate(), billingReportInput.getToPostingDate());
	invoiceHeaderRepository.updateARInvoiceCostBilled(billingReportInput.getClassId(),billingReportInput.getFromPostingDate(), billingReportInput.getToPostingDate());
