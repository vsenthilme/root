...
//			@Param(value = "matterNumber") String matterNumber);
    ...
    @Query(value = "insert into tblarreportpu (paid_amount,matter_no) (select sum(COALESCE(pu.PAYMENT_AMOUNT,0)) paid_amount, matter_no from tblpaymentupdate pu \n"
            + "where \n"
//            + "pu.class_id IN (:classId) and \n"
            + "pu.payment_date between :fromDate and :toDate group by matter_no); ", nativeQuery = true)
...
//            @Param(value = "classId") List<Long> classId,
    ...
    @Modifying
    @Transactional
    @Query(value = "insert into tblarreportinv (matter_no,invoice_amount) (select matter_no,sum(COALESCE(invoice_amt,0)) invoice_amount from tblinvoiceheader where is_deleted = 0 and \n"
//            + "class_id IN (:classId) and \n"
...
//            @Param(value = "classId") List<Long> classId,
    ...
    @Query(value = "UPDATE tblarreportinv inv \n"
            + "JOIN ( \n"
            + "    SELECT matter_no, SUM(COALESCE(bill_amount,0)) AS fee_billed \n"
            + "    FROM tblinvoiceline \n"
            + "    WHERE item_no = 1 and \n"
//            + "    class_id IN (:classId) and  \n"
            + "    ctd_on between :fromDate and :toDate \n"
...
//            @Param(value = "classId") List<Long> classId,
    ...
    @Query(value = "UPDATE tblarreportinv inv \n"
            + "JOIN ( \n"
            + "    SELECT matter_no, SUM(COALESCE(bill_amount,0)) AS cost_billed \n"
            + "    FROM tblinvoiceline \n"
            + "    WHERE item_no = 2 and \n"
//            + "    class_id IN (:classId) and \n"
            + "    ctd_on between :fromDate and :toDate \n"
