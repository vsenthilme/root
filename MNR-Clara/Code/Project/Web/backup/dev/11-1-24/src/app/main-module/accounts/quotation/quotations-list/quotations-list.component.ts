  ...
  //uploadInvoiceQuotation
        if(page == 'New'){
          ...
          pdfDocGenerator.getBlob((blob) => {
            var file = new File([blob], 'Quotation No - ' + res.quotationNo + ".pdf"); //  + (new Date().getDate()) +'-'+ (new Date().getMonth() + 1) + '-' + new Date().getFullYear() + '_' + this.cs.timeFormat(new Date()) +  
            if (file) {
              this.spin.show();
              this.service.uploadfile(file, 'Quotation/' + element.matterNumber).subscribe((resp: any) => {
                let obj: any = {};
                res.referenceField1 = resp.file;
                this.service.Update(res, element.quotationNo, element.quotationRevisionNo).subscribe(res => {
                  this.spin.hide();
                })
              })
            }
