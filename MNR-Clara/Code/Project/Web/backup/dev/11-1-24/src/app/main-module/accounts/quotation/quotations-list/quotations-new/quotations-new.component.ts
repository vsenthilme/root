...
export class QuotationsNewComponent implements OnInit {
  ...
  constructor(
    ...
    private Quotation: QuotationsListComponent
  ...
  ngOnInit(): void {
    ...
    this.QuotationLine.controls.quantity.valueChanges.subscribe((x) => {
      ...
      if (x)
        this.QuotationLine.controls.totalAmount.patchValue(
          ((this.QuotationLine.controls.quantity.value as number) *
            this.QuotationLine.controls.rateperHour.value) as number
        );
    });
  ...
  ngdropdown() {
      ...
      this.service.GetClientdetails().subscribe(
        (res) => {
          ...
          this.clientList.forEach(
            (x: { clientId: string; firstNameLastName: string }) =>
    ...
    this.sub.add(
      this.service.getmatter().subscribe(
        (res) => {
          this.matterList = res;
          ...
          this.matterList.forEach((x: { matterNumber: string }) =>
            this.multimatterList.push({
              value: x.matterNumber,
              label: x.matterNumber,
            })
          );
  ...
  get_clientetails(code: any) {
    ...
    this.spin.show();
    this.sub.add(
      this.serviceMatter.GetClient(this.form.controls.clientId.value).subscribe(
        (res) => {
          this.form.controls.referenceField9.patchValue(
            res.corporationClientId
          );
          this.spin.hide();
        },
        (err) => {
  ...
  submit(x: any = null) {
    ...
    if (
      x &&
      this.form.controls.statusId.value != 10 &&
      this.pageflow != 'New'
    ) {
    ...
    if (this.rows.controls.length == 0) {
      this.toastr.error('Please add line data.', 'Notification', {
        timeOut: 2000,
        progressBar: false,
      });
    ...
    if (this.form.controls.quotationNo.value) {
      this.sub.add(
        this.service
          .Update(
            ...
            this.form.controls.quotationNo.value,
            this.form.controls.quotationRevisionNo.value
          )
          .subscribe(
            (res) => {
              this.toastr.success(
                this.form.controls.quotationNo.value + ' updated successfully!',
                'Notification',
                {
                  timeOut: 2000,
                  progressBar: false,
      ...
      this.sub.add(
        this.service.Create(this.form.getRawValue()).subscribe(
          (res) => {
            this.toastr.success(
              res.quotationNo + ' saved successfully!',
              'Notification',
              {
                timeOut: 2000,
                progressBar: false,
            ...
            this.Quotation.generatePdf(res, 'New'); //uploadInvoiceQuotation
            this.router.navigate(['/main/accounts/quotationslist']);
          },
          (err) => {
            this.cs.commonerror(err);
  ...
  addRow(d?: any, noUpdate?: boolean) {
    ...
    row.controls.quantity.valueChanges.subscribe((x) => {
      ...
      if (x)
        row.controls.totalAmount.patchValue(
          ((row.controls.quantity.value as number) *
            row.controls.rateperHour.value) as number
        );
    });
