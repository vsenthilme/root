...
} from '../../chequerequest/chequerequest.service';
import {
...
export class ExpensesChequeComponent implements OnInit {
  ...
  public mask1 = [/\d/, /\d/, /\d/, /\d/,  ' ', /\d/, /\d/, /\d/, /\d/,  ' ', /\d/, /\d/, /\d/, /\d/,  ' ', /\d/, /\d/, /\d/, /\d/,  ' ', /\d/, /\d/, /\d/, /\d/]
  ...
  ngOnInit(): void {
    ...
    this.sub.add(this.clientService.Get(this.js.data.clientId).subscribe(res => {
      this.form.controls.clientName.patchValue(res.lastNameFirstName);
  ...
  submit() {
    if (this.js.pageflow == 'Approve') {
      ...
      this.submitForm('raj@innovitsolution.com');  //accounting@montyramirezlaw.com
    }
