...
export class TimeTicketsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // if(this.code.opentabNew == true){
    //   this.openNewTabDialog(this.code.pageflow)
    // }
  ...
  // openNewTabDialog(data){
  //   const dialogRef = this.dialog.open(TimeNewComponent, {
  //     width: '70%',
  //     maxWidth: '80%',
  //     // position: { top: '6.5%' },
  //     data: { pageflow: data, matter: this.code.matter, matterdesc: this.code.matterdesc, code: data != 'New' ? this.code.code : null }
  //   });
  //   dialogRef.afterClosed().subscribe(result => {
  //     //this.getAllListData();
  //     //   window.location.reload();
  ...
  //     paramdata = this.cs.encrypt({opentabNew: true, matter: this.selection.selected[0].matterNumber, matterdesc: this.matterdesc, pageflow: data, code: data != 'New' ? this.selection.selected[0].timeTicketNumber : null});

  //        const url = this.router.serializeUrl(
  //         this.router.createUrlTree(['/main/matters/case-management/timeticket/' + paramdata])
  //       );
