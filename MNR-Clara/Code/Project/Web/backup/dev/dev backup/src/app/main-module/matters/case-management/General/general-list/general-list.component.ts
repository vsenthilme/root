...
export class GeneralListComponent implements OnInit {
  ...
  search() {
    ...
    this.sub.add(this.service.Search(this.searhform.getRawValue()).subscribe((res: GeneralMatterElement[]) => {
      ...
      this.dataSource = new MatTableDataSource < GeneralMatterElement > (this.ELEMENT_DATA);
      this.selection = new SelectionModel < GeneralMatterElement > (true, []);
      this.dataSource.sort = this.sort;
  ...
  downloadexcel() {
    ...
    this.sub.add(this.service.Search(this.searhform.getRawValue()).subscribe((data) => {
      data.forEach(x => {
        res.push({
          "Class": x.referenceField27,
          "Matter No": x.matterNumber,
          'Matter Description': x.matterDescription,
          "Client Name ": x.referenceField26,
          "Case Info Sheet No": x.caseInformationNo,
          'Case Category': x.referenceField28,
          'Case Sub Category': x.referenceField24,
          "Status  ": x.referenceField23,
          'Case Opened Date': this.cs.dateapi(x.caseOpenedDate),
          
          "referenceField4  ": x.referenceField4,
        });
      })
