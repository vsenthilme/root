...
this.getAllMatterCheckList();
  ...
  getAllMatterCheckList() {
    ...
    this.cas.getalldropdownlist([
      ...
      this.sub.add(this.service.getAllMatterDocList(this.code).subscribe(listData => {
        ...
        listData =  listData.filter(x => x.deletionIndicator != 1)   ///YETTOBEPROD
  ...
  delete() {
    if (this.selection.selected.length > 0) {
      ...
      this.spin.show();
      this.sub.add(this.service.deleteMatterChecklistDocument(this.selection.selected[0]).subscribe(res => {
        this.getAllMatterCheckList();
