...
//			@Param(value = "matterNumber") String matterNumber);
	...
	@Query(value="SELECT REF_FIELD_1 AS preBillNumber, TK_CODE AS attorney, SUM(APP_BILL_TIME) AS hoursBilled, SUM(APP_BILL_AMOUNT) AS amountBilled \r\n"
			...
			+ " WHERE MATTER_NO = :matterNumber and REF_FIELD_1 = :preBillNumber \r\n"
			+ " AND TK_CODE IN :tkCode GROUP BY REF_FIELD_1, TK_CODE", nativeQuery = true)
	...
	@Query(value = "select *,(x.softCost+x.hardCost) totalCost,(x.totalBilled-COALESCE(x.paidAmount,0)) balance from \n"
			...
			+ "class_id_desc classDescription,\n"
			+ "client_no clientId,\n"
			+ "client_name clientName,\n"
			+ "matter_no matterNumber,\n"
			+ "matter_name matterDescription,\n"
			+ "partner,\n"
			+ "responsible_tk responsibleTimekeeper,\n"
			+ "assigned_tk assignedTimekeeper,\n"
			+ "(select sum(COALESCE(PAYMENT_AMOUNT,0)) from tblpaymentupdate where matter_no = :matterNumber and payment_date between :fromDate and :toDate) paidAmount,\n"
			+ "sum(COALESCE(total_billed,0)) totalBilled,\n"
			+ "sum(COALESCE(soft_cost,0)) softCost,\n"
			+ "sum(COALESCE(hard_cost,0)) hardCost,\n"
			+ "sum(COALESCE(fee_billed,0)) feeBilled\n"
			+ "from tblbillingreport\n"
			+ "where\n"
			+ "(COALESCE(:classId) IS NULL OR (class_id IN (:classId))) and\n"
			+ "(COALESCE(:clientId) IS NULL OR (client_no IN (:clientId))) and\n"
			+ "(COALESCE(:matterNumber) IS NULL OR (matter_no IN (:matterNumber))) and \n"
			+ "(COALESCE(:partner) IS NULL OR (partner IN (:partner))) and  \n"
			+ "(COALESCE(:responsibleTimeKeeper) IS NULL OR (responsible_tk IN (:responsibleTimeKeeper))) and  \n"
			+ "(COALESCE(:assignedTimeKeeper) IS NULL OR (assigned_tk IN (:assignedTimeKeeper))) and  \n"
			+ "(COALESCE(:fromDate) IS NULL OR (posting_date between :fromDate and :toDate)) \n"
