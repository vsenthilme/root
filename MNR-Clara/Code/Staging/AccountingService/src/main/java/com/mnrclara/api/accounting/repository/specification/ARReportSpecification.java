...
public class ARReportSpecification implements Specification<ARReport> {
    ...
    public Predicate toPredicate(Root<ARReport> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		 ...
		 if (searchARReport.getFromDate() != null && searchARReport.getToDate() != null) {
             predicates.add(cb.between(root.get("postingDate"), searchARReport.getFromDate(), searchARReport.getToDate()));
         }
