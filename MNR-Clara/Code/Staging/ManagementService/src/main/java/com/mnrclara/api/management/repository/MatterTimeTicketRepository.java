...
//												   @Param ("classId") List<Long> classId);
	...
	public ITimeKeeperBillableHoursReport findTimeKeeperTimeTicketHoursBillable (@Param ("timeKeeperCode") String timeKeeperCode,
	...
	@Query(value = "select \n"
			...
			+ "tt.tk_code IN (:timeKeeperCode) and tt.class_id IN (:classId) and \n"
			+ "(COALESCE(:startDate,null) IS NULL OR (tt.time_ticket_date BETWEEN :startDate AND :endDate)) and \n"
			+ "tt.status_id=51 and tt.is_deleted=0 and tt.bill_type in ('Non-Billable')", nativeQuery = true)
...
//	@Query(value = "select \n"
//			+ "sum(invoice_amt) billedFees \n"
//			+ "from tblinvoiceheader ih \n"
//			+ "where \n"
//			+ "ih.partner_assigned IN (:timeKeeperCode) and \n"
//			+ "(COALESCE(:startDate,null) IS NULL OR (ih.invoice_date BETWEEN :startDate AND :endDate)) and \n"
//			+ "ih.is_deleted=0", nativeQuery = true)
//	public ITimeKeeperBillableHoursReport findTimeKeeperInvoice (@Param ("timeKeeperCode") String timeKeeperCode,
//																	   @Param(value = "startDate") Date startDate,
//																	   @Param(value = "endDate") Date endDate);
