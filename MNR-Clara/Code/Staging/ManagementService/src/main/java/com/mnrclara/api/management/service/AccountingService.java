...
//		log.info("hourlyByGroupMatterGenerals:-------> " + hourlyByGroupMatterGenerals);
		
		if (!hourlyByGroupMatterGenerals.isEmpty()) {
			List<IMatterTimeTicketCountAndSum> matterTimeTicketsCountAndSum = 
					matterTimeTicketRepository.findCountAndSumOfTimeTicketsByGroup(matterNumbers, startDate, feesCutoffDate, originatingTimeKeeper, responsibleTimeKeeper, assignedTimeKeeper);
			List<IMatterExpenseCountAndSum> matterExpenseTicketsCountAndSum = 
					matterExpenseRepository.findExpenseByGroup(matterNumbers, startDate, feesCutoffDate, originatingTimeKeeper, 
...
//			log.info("contigencyByGroupMatterGenerals:-------> " + contigencyByGroupMatterGenerals);
			if (!contigencyByGroupMatterGenerals.isEmpty()) {
				List<IMatterTimeTicketCountAndSum> matterTimeTicketsCountAndSum = 
						matterTimeTicketRepository.findCountAndSumOfTimeTicketsByGroup(matterNumbers, startDate, feesCutoffDate, 
...
//				log.info("filteredMatterNumbers---------> : " + filteredMatterNumbers);
		...
		if (!hourlyByGroupMatterGeneralsForClass2.isEmpty()) {
			...
			List<String[]> matterTimeTicketsCountAndSum = 
					matterTimeTicketRepository.findCountAndSumOfTimeTicketsByGroup1 (matterNumbers, startDate, feesCutoffDate, originatingTimeKeeper, responsibleTimeKeeper, assignedTimeKeeper);
			List<String[]> matterExpenseTicketsCountAndSum = 
					matterExpenseRepository.findExpenseByGroup1(matterNumbers, startDate, feesCutoffDate, originatingTimeKeeper, responsibleTimeKeeper, assignedTimeKeeper);
		...
		if (matterTimeTicketsCountAndSum != null && !matterTimeTicketsCountAndSum.isEmpty()) {
			...
			try {
				for (IMatterTimeTicketCountAndSum timeTicket : matterTimeTicketsCountAndSum) {
					...
					if (matterGenAcc.get().getBillingModeId().equalsIgnoreCase("2")) {
						if (matterGenAcc.get().getRemainingAmount() != 0) {
				...
				matterNumbers.forEach(m -> {
					...
					if (!matterGenAccOpt.isEmpty()) {
						...
						if (matterGenAcc.getBillingModeId().equalsIgnoreCase("2")) {
							if (matterGenAcc.getRemainingAmount() != 0) {
