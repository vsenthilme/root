...
public class MatterGenAccService {
     ...
     *
     * @param attorneyProductivityInput
     * @return
     * @throws ParseException
     */
    public List<AttorneyProductivityReport> getAttorneyProductivityReport(AttorneyProductivityInput attorneyProductivityInput)
            throws ParseException {
        ...
        if (attorneyProductivityInput.getFromFiledDate() != null && attorneyProductivityInput.getToFiledDate() != null) {
            Date fromCaseFiledDate = DateUtils.convertStringToDate3(attorneyProductivityInput.getFromFiledDate());
            Date toCaseFiledDate = DateUtils.convertStringToDate3(attorneyProductivityInput.getToFiledDate());
            dates = DateUtils.addTimeToDatesForSearch(fromCaseFiledDate, toCaseFiledDate);
            attorneyProductivityInput.setCaseFiledStartDate(dates[0]);
            attorneyProductivityInput.setCaseFiledEndDate(dates[1]);
        }
        ...
        for (AttorneyProductivityReportImpl newAttorneyProductivityReport : dbAttorneyProductivityReportList) {
            ...
            /*
             * 1. Fetch Legal assistant and REF_FIELD_2 from MATTER_ASSIGNMENT table by passing MATTER_NO and add the columns in response
             */
            Optional<MatterAssignment> optMatterAssignment = matterAssignmentRepository.findByMatterNumber(newAttorneyProductivityReport.getMatterNumber());
            if (!optMatterAssignment.isEmpty()) {
                MatterAssignment matterAssignment = optMatterAssignment.get();
                dbAttorneyProductivityReport.setLegalAssistant(matterAssignment.getLegalAssistant());
                dbAttorneyProductivityReport.setParaLegal(matterAssignment.getReferenceField2());
            }

            // 2. Pass MATTER_NO and date given in the input into INVOICEHEADER table and fetch SUM of INV_AMOUNT and set
            Double invoiceAmount = invoiceHeaderRepository.getSumOfInvoiceAmountByMatterNumber(newAttorneyProductivityReport.getMatterNumber(), dates[0], dates[1]);
            dbAttorneyProductivityReport.setInvoiceAmount(invoiceAmount);
